...
public:
    ...
    void add_light_projection(const Eigen::Matrix4f& p, const Eigen::Matrix4f &view, const light l);
    void set_shadow_buffer(const std::vector<float>& shadow_buffer);
    void shadow_mapping(const std::vector<Triangle *> triangles, int light_ind);
...
private:
    ...
    void rasterize_triangle(const Triangle& t,
                            ...
                            Shading shading = Shading::Phong, bool shadow=false);
    void rasterize_triangle_shadow(const Triangle& t,
                               const std::array<Eigen::Vector3f, 3>& view_pos,
                               const std::array<Eigen::Vector3f, 3>& world_pos,
                               const std::vector<light>& viewspace_lights);

    void post_process_buffer();
    float distance(Vector3f x1, Vector3f x2);
...
private:
    ...
    std::vector<Eigen::Matrix4f> shadow_views;
    std::vector<Eigen::Matrix4f> light_projections;
    ...
    std::vector<Vector3f> shadow_map;
    std::vector<Eigen::Vector3f> ssaa_frame_buf;
    std::vector<float> ssaa_depth_buf;
    std::vector<std::vector<float>> shadow_bufs;
    std::array<Eigen::Vector3f, 3> worldspace_pos;
